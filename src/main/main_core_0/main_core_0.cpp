...
void core_0_task(void* args) {
  QueueHandle_t controlOutputQueue = static_cast<QueueHandle_t>(args);
  ...
  while (true) {
    ...
    my_data.voltage = 0.0F;  // my_ina.get_voltage();
    my_data.current = 0.0F;  // my_ina.get_current();
    my_data.power = 0.0F;    // my_ina.get_power();
